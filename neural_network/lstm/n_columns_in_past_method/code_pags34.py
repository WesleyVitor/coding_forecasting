...
max_days = 90
df_pags34_original = pags34.history(start=start-relativedelta(days=max_days*2),end=end)

df_pags34 = df_pags34_original
# Removendo as colunas que não será necessárias
df_pags34 = df_pags34.drop(columns=['Open', 'High', 'Low', 'Volume', 'Dividends', 'Stock Splits'])
...
for epoch in range(100, 700, 100):
  ...
  for janela in range(15, max_days+15, 15):
      ...
      for column_id in range(1, janela+1):
          df_pags34[[f'days_before_{column_id}']] = df_pags34_original[['Close']].shift(column_id)
      # Listagem do nome das novas colunas que foram inseridas
      columns_inserted = list(df_pags34.columns)
      columns_inserted.pop(0) #Remove Close column


      # Volta para o range que estamos estudando 2021-2023
      df_pags34 = df_pags34.loc[df_pags34.index > '2021-08-01']
      total_rows = len(df_pags34)


      # Divisão de elementos de treinamento e teste
      df_train = df_pags34.iloc[:total_rows - legs]
      df_test = df_pags34.iloc[total_rows - legs:]
