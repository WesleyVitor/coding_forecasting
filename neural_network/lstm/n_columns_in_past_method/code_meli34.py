...
max_days = 90
df_meli34_original = meli34.history(start=start-relativedelta(days=max_days*2),end=end)
...
for epoch in range(100, 700, 100):
  ...
  for janela in range(15, max_days+15, 15):
      ...
      for column_id in range(1, janela+1):
          df_meli34[[f'days_before_{column_id}']] = df_meli34_original[['Close']].shift(column_id)
      # Listagem do nome das novas colunas que foram inseridas
      columns_inserted = list(df_meli34.columns)
      columns_inserted.pop(0) #Remove Close column


      # Volta para o range que estamos estudando 2021-2023
      df_meli34 = df_meli34.loc[df_meli34.index > '2021-08-01']
